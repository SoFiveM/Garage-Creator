...
Citizen.CreateThread(function()
    while true do
        time = 500
        if inGarage and Config.scaleform then 
            time = 0
...
function loadvehicles(id, currentPlaces)
    ESX.TriggerServerCallback('pl_checkcars', function(result)
        if result ~= "[]" then 
            Config.currentVehicleingarage = {}
            for k, v in pairs(result) do 
                local vehicleProps = json.decode(v.vehicleprops)
                ESX.Game.SpawnLocalVehicle(vehicleProps.model, Config.GarageList[currentPlaces..'place'].CarSpots[k].Pos, Config.GarageList[currentPlaces..'place'].CarSpots[k].Heading, function(vehicle) 
                    ESX.Game.SetVehicleProperties(vehicle, vehicleProps)
                    table.insert(Config.currentVehicleingarage, {vehicle = vehicle, carid = v.id, garageid = id, props = vehicleProps})
...
function rangementvehicule(id)
    ...
    ESX.TriggerServerCallback("pl_checkcarsid", function(cb)
        if cb ~= nil then 
            ESX.ShowNotification("~r~Il y a déja une plaque enregistrers au m'aime nom.")
        else
